    ...
    <main className="h-screen flex flex-col items-center justify-between">
      <header className="mt-6 w-[940px] fixed flex gap-6 p-5 top-0 left-1/2 -translate-x-1/2 z-10 bg-[#191919] rounded border-[#3c3c3c] shadow-md">
        ...
        <div className="export-btn self-center ml-auto">
          <button
            className="flex items-center gap-3 py-2 px-3 rounded-md text-sm bg-blue-400 text-blue-400 font-medium bg-opacity-10 hover:bg-opacity-80 hover:text-slate-50 ease-in-out transition-all duration-300"
            onClick={exportPng}
          >
            <Download />
            Export PNG
      ...
      <div className="code-editor-ref mt-56" ref={editorRef}>
        <CodeEditor
          language={language}
          icon={activeIcon}
          theme={theme}
